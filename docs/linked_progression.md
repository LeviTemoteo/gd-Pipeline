...
## Independent Storage
Every JSON file generated by Death Tracker and Playtime Tracker remains an independent row inside SQLite.
This preserves the original data source and keeps the database fully traceable, then aggregation is performed only during synchronization.
