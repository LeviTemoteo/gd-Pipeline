...
The pipeline is triggered whenever the File Watcher detects a modification in one of the monitored JSON files.
...
Responsible for monitoring the JSON files generated by Geode mods.
...
The pipeline only runs when one of the monitored JSON files changes.
